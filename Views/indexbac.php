...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Create instance of UserC
    $userC = new UserC();
    
    // Try to log the user in
    $user = $userC->loginUser($email, $password);

    if ($user) {
        // Check if the user has the "admin" role
        if ($user['role'] == 'admin') {
            $_SESSION['user'] = $user; // Store user data in the session
            header("Location: listUser.php"); // Redirect to listUser.php
            exit;
        } else {
            $_SESSION['error'] = "You are not an admin."; // Store error message
        }
    } else {
        $_SESSION['error'] = "Invalid email or password."; // Invalid login credentials
    }
}
?>
